...
import { DEFAULT_QUERY_PARAMS } from "../../public/pages/ManagedIndices/utils/constants";
...
export default class ManagedIndexService {
  ...
  getManagedIndices = async (req: Request, h: ResponseToolkit): Promise<ServerResponse<GetManagedIndicesResponse>> => {
    try {
      ...
      if (!indices.length) {
        return { ok: true, resp: { managedIndices: [], totalManagedIndices: 0 } };
      }
      ...
      return { ok: true, resp: { managedIndices, totalManagedIndices } };
    } catch (err) {
