...
export default class PolicyService {
  ...
  getPolicies = async (req: Request, h: ResponseToolkit): Promise<ServerResponse<GetPoliciesResponse>> => {
    try {
      ...
      // TODO change policy.description to policy.policy_id when it's available from backend fix
      const policySorts: PoliciesSort = { description: "policy.description.keyword" };
      const params = {
        ...
        body: {
          ...
          query: {
            bool: {
              ...
              // TODO change policy.description to policy.policy_id when it's available from backend fix
              must: getMustQuery("policy.description", search),
            },
          },
        },
      };
